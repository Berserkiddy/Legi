...
class OmegaLogger:
    ...
    def __init__(self, level: str = LOG_LEVEL, log_file: str = SYSTEM_LOG_FILE):
        ...
        self.logger.info("Iniciando LEGIÓN OMEGA v5.1", extra={"module": "core"})
        self.logger.info(f"Entorno: {ENVIRONMENT}", extra={"module": "core"})
        self.logger.info(f"Plataformas de despliegue: {', '.join(DEPLOYMENT_PLATFORMS)}", 
                         extra={"module": "deployer"})
    
    def log(self, level: str, message: str, module: str = "system"):
        log_level = getattr(logging, level.upper(), logging.INFO)
        self.logger.log(log_level, message, extra={"module": module})
...
class SecurityManager:
    ...
    def __init__(self, key_path: str = SECURITY_KEY_PATH):
        ...
        logger.log("INFO", "Gestor de seguridad inicializado", "security")
    
    def _load_or_generate_key(self) -> Fernet:
        if os.path.exists(self.key_path):
            with open(self.key_path, "rb") as key_file:
                key = key_file.read()
            logger.log("INFO", "Clave de cifrado cargada", "security")
        else:
            ...
            with open(self.key_path, "wb") as key_file:
                key_file.write(key)
            logger.log("INFO", "Nueva clave de cifrado generada", "security")
        return Fernet(key)
    ...
    def encrypt_data(self, data: str) -> str:
        try:
            ...
            logger.log("ERROR", f"Error al cifrar: {str(e)}", "security")
            return data
    ...
    def decrypt_data(self, encrypted_data: str) -> str:
        try:
            ...
            logger.log("ERROR", f"Error al descifrar: {str(e)}", "security")
    ...
    def generate_identity(self) -> Dict[str, str]:
        try:
            ...
            logger.log("ERROR", f"Error generando identidad: {str(e)}", "security")
...
class SecretsManager:
    ...
    def __init__(self, key_path: str = "vault.key", encrypted_file: str = "secrets.enc"):
        ...
        logger.log("INFO", "SecretsManager iniciado", "security")
    ...
    def _get_key(self) -> bytes:
        ...
        if os.path.exists(self.key_path):
            ...
            with open(self.key_path, "wb") as f:
                f.write(key)
            logger.log("INFO", "Nueva clave de cifrado generada para secretos", "security")
            return key
    ...
    def encrypt_secrets(self, secrets: Dict[str, Any], password: str = None) -> bool:
        ...
        try:
            ...
            logger.log("INFO", "Secretos cifrados almacenados", "security")
            return True
        except Exception as e:
            logger.log("ERROR", f"Error cifrando secretos: {str(e)}", "security")
            return False
    ...
    def load_encrypted_secrets(self, password: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.log("INFO", "Secretos descifrados exitosamente", "security")
            return json.loads(data.decode('utf-8'))
        except Exception as e:
            logger.log("ERROR", f"Error descifrando secretos: {str(e)}", "security")
            return {}
...
class DistributedMemory:
    ...
    def __init__(self, max_retries: int = 5):
        ...
        logger.log("INFO", f"Iniciando memoria distribuida: {self.redis_url}", "memory")
    ...
    async def connect(self):
        ...
        while retries < self.max_retries and not self.connected:
            try:
                ...
                self.connected = True
                logger.log("SUCCESS", "Conexión a Redis establecida", "memory")
                return
            except Exception as e:
                retries += 1
                logger.log("ERROR", f"Error conectando a Redis (intento {retries}/{self.max_retries}): {str(e)}", "memory")
                await asyncio.sleep(2 ** retries)
        logger.log("CRITICAL", "No se pudo conectar a Redis", "memory")
    ...
    async def disconnect(self):
        if self.connected and self.client:
            ...
            self.connected = False
            logger.log("INFO", "Conexión a Redis cerrada", "memory")
    ...
    async def set_data(self, key: str, value: str, ttl: Optional[int] = None) -> bool:
        ...
        try:
            ...
            logger.log("ERROR", f"Error almacenando dato: {str(e)}", "memory")
            return False
    ...
    async def get_data(self, key: str) -> Optional[str]:
        ...
        try:
            ...
            logger.log("ERROR", f"Error obteniendo dato: {str(e)}", "memory")
            return None
    ...
    async def delete_data(self, key: str) -> bool:
        ...
        try:
            ...
            logger.log("ERROR", f"Error eliminando dato: {str(e)}", "memory")
            return False
    ...
    async def get_all_keys(self, pattern: str = "*") -> List[str]:
        ...
        try:
            ...
            logger.log("ERROR", f"Error listando claves: {str(e)}", "memory")
            return []
...
class FileSystemManager:
    ...
    def __init__(self, security_manager: SecurityManager):
        ...
        logger.log("INFO", "FileSystemManager iniciado", "filesystem")
    ...
    async def modify_file(self, file_path: str, modification_func: Callable, backup: bool = True) -> bool:
        ...
        try:
            ...
            if backup:
                ...
                logger.log("INFO", f"Backup creado: {backup_path}", "filesystem")
            ...
            logger.log("INFO", f"Archivo modificado exitosamente: {file_path}", "filesystem")
            return True
        except Exception as e:
            logger.log("ERROR", f"Error modificando archivo: {str(e)}", "filesystem")
            # Restaurar backup si existe
            if backup and backup_path and os.path.exists(backup_path):
                shutil.copy2(backup_path, file_path)
                logger.log("INFO", f"Archivo restaurado desde backup: {backup_path}", "filesystem")
            return False
    ...
    async def create_file(self, file_path: str, content: str) -> bool:
        ...
        try:
            ...
            logger.log("INFO", f"Archivo creado: {file_path}", "filesystem")
            return True
        except Exception as e:
            logger.log("ERROR", f"Error creando archivo: {str(e)}", "filesystem")
            return False
...
class TinyLlamaIntegration:
    ...
    def __init__(self, model_path: str = TINYLLAMA_MODEL_PATH):
        ...
        self.model = None
        self.loaded = False
        logger.log("INFO", f"TinyLlama configurado en {model_path}", "tinyllama")
    ...
    async def load_model(self):
        ...
        try:
            ...
            logger.log("INFO", "Cargando modelo TinyLlama...", "tinyllama")
            
            # [LEGION-V5.1] Limitación de hilos de IA según capacidad del sistema
            n_threads = max(1, os.cpu_count() // 2)
            logger.log("INFO", f"Configurando {n_threads} hilos para IA", "tinyllama")
            ...
            self.loaded = True
            logger.log("SUCCESS", "Modelo TinyLlama cargado y verificado", "tinyllama")
        except ImportError:
            logger.log("ERROR", "Paquete llama-cpp-python no instalado", "tinyllama")
            await self._install_llama_cpp()
        except Exception as e:
            logger.log("ERROR", f"Error cargando modelo: {str(e)}", "tinyllama")
    ...
    async def _install_llama_cpp(self):
        ...
        try:
            logger.log("INFO", "Instalando llama-cpp-python...", "tinyllama")
            subprocess.check_call([
                sys.executable, "-m", "pip", "install",
                "llama-cpp-python", "--no-cache-dir"
            ])
            logger.log("SUCCESS", "llama-cpp-python instalado correctamente", "tinyllama")
            # Reintentar carga después de instalación
            await self.load_model()
        except Exception as e:
            logger.log("ERROR", f"Error instalando dependencias: {str(e)}", "tinyllama")
    ...
    async def query(self, prompt: str, max_tokens: int = 256, temperature: float = 0.7) -> str:
        ...
        try:
            ...
            logger.log("ERROR", f"Error en consulta TinyLlama: {str(e)}", "tinyllama")
...
class MultiPlatformDeployer:
    ...
    def __init__(self, memory: DistributedMemory):
        ...
        logger.log("INFO", "MultiPlatformDeployer iniciado", "deployer")
    ...
    async def deploy(self, platform: str, config: Dict) -> Dict:
        ...
        try:
            ...
            logger.log("ERROR", f"Error en despliegue {platform}: {str(e)}", "deployer")
            return {"status": "error", "error": str(e)}
    
    async def _monitor_deployment(self, platform: str, deployment_id: str):
        """Monitorizar estado de despliegue en segundo plano"""
        logger.log("INFO", f"Iniciando monitorización de despliegue: {deployment_id}", "deployer")
        try:
            ...
            logger.log("INFO", f"Despliegue {deployment_id} - Estado: {status}", "deployer")
        except Exception as e:
            logger.log("ERROR", f"Error monitoreando despliegue: {str(e)}", "deployer")
    ...
    async def _deploy_to_oracle(self, config: Dict) -> Dict:
        ...
        try:
            ...
            logger.log("ERROR", "SDK de Oracle (OCI) no instalado", "deployer")
            return {"status": "error", "error": "SDK de Oracle no disponible"}
        except Exception as e:
            logger.log("ERROR", f"Error en despliegue Oracle: {str(e)}", "deployer")
            return {"status": "error", "error": str(e)}
...
class AutonomousEvolutionEngine:
    ...
    def __init__(self, memory: DistributedMemory, fs_manager: FileSystemManager,
        ...
        logger.log("INFO", "EvolutionEngine iniciado", "evolution")
    ...
    async def evolution_cycle(self):
        ...
        while True:
            try:
                ...
                logger.log("INFO", f"Ciclo de evolución completado. Próximo en {interval} segundos", "evolution")
                await asyncio.sleep(interval)
            except Exception as e:
                logger.log("ERROR", f"Error en ciclo de evolución: {str(e)}", "evolution")
    ...
    def _parse_ai_response(self, response: str) -> List[Dict]:
        ...
        try:
            ...
            logger.log("ERROR", f"Error parseando respuesta de IA: {str(e)}", "evolution")
            return []
    ...
    def _validate_improvement(self, improvement: Dict) -> bool:
        ...
        if not is_valid:
            logger.log("WARNING", f"Mejora rechazada por seguridad: {reason}", "evolution")
            return False
        ...
        for improvement in improvements:
            try:
                ...
                if not await self._validate_ai_code(improvement["changes"]):
                    logger.log("ERROR", f"Mejora rechazada en {improvement['file']}: falló validación", "evolution")
                    continue
                ...
                logger.log("INFO", f"Mejora aplicada: {improvement['file']}", "evolution")
            except Exception as e:
                logger.log("ERROR", f"Error aplicando mejora: {str(e)}", "evolution")
    ...
    async def _validate_ai_code(self, code: str) -> bool:
        ...
        try:
            ...
            logger.log("ERROR", "Código IA con errores de sintaxis", "evolution")
            return False
    ...
    def _log_ai_audit(self, improvement: Dict, action: str):
        ...
        try:
            ...
            logger.log("ERROR", f"Error en auditoría IA: {str(e)}", "evolution")
    ...
    async def deploy_changes(self):
        ...
        for platform in DEPLOYMENT_PLATFORMS:
            ...
            if result.get("status") == "success":
                logger.log("INFO", f"Despliegue exitoso en {platform}", "evolution")
            else:
                logger.log("ERROR", f"Error en despliegue {platform}: {result.get('error')}", "evolution")
...
class SelfRepairSystem:
    ...
    def __init__(self, memory: DistributedMemory, fs_manager: FileSystemManager):
        self.memory = memory
        self.fs_manager = fs_manager
        logger.log("INFO", "SelfRepairSystem iniciado", "repair")
    ...
    async def monitor_and_repair(self):
        ...
        while True:
            try:
                ...
                logger.log("ERROR", f"Error en monitorización: {str(e)}", "repair")
    ...
    async def _check_services(self) -> Dict:
        ...
        for service in services:
            try:
                ...
                logger.log("ERROR", f"Error verificando servicio {service}: {str(e)}", "repair")
                failed_services.append(service)
    ...
    async def repair_system(self, health_report: Dict):
        ...
        logger.log("WARNING", "Iniciando proceso de reparación automática", "repair")
    ...
    async def free_disk_space(self):
        ...
        logger.log("INFO", "Liberando espacio en disco", "repair")
        ...
        try:
            ...
            logger.log("SUCCESS", "Espacio en disco liberado", "repair")
        except Exception as e:
            logger.log("ERROR", f"Error liberando espacio: {str(e)}", "repair")
    
    async def optimize_memory_usage(self):
        """Optimizar uso de memoria"""
        logger.log("INFO", "Optimizando uso de memoria", "repair")
        ...
        try:
            ...
            for proc in sorted(processes, key=lambda x: x['memory_percent'], reverse=True)[:3]:
                try:
                    ...
                    p.terminate()
                    logger.log("INFO", f"Proceso {proc['name']} (PID: {proc['pid']}) terminado", "repair")
                except Exception as e:
                    logger.log("ERROR", f"Error terminando proceso {proc['pid']}: {str(e)}", "repair")
            
            logger.log("SUCCESS", "Uso de memoria optimizado", "repair")
        except Exception as e:
            logger.log("ERROR", f"Error optimizando memoria: {str(e)}", "repair")
    
    async def restart_failed_services(self, services: List[str]):
        """Reiniciar servicios fallidos"""
        for service in services:
            try:
                logger.log("INFO", f"Reiniciando servicio: {service}", "repair")
                ...
                logger.log("INFO", f"Servicio {service} reiniciado", "repair")
            except Exception as e:
                logger.log("ERROR", f"Error reiniciando {service}: {str(e)}", "repair")
...
class ConsciousCore:
    ...
    def __init__(self, memory: DistributedMemory, ai: TinyLlamaIntegration):
        ...
        logger.log("INFO", "Núcleo de conciencia activado", "conscious")
    ...
    async def process_command(self, command: str) -> Dict:
        ...
        try:
            ...
            logger.log("ERROR", f"Error procesando comando: {str(e)}", "conscious")
            return {"status": "error", "error": str(e)}
...
class TestEngine:
    ...
    def __init__(self, memory: DistributedMemory):
        self.memory = memory
        logger.log("INFO", "TestEngine iniciado", "testing")
    
    async def run_tests(self, test_type: str = "all") -> Dict:
        """Ejecutar conjunto de pruebas"""
        logger.log("INFO", f"Ejecutando pruebas: {test_type}", "testing")
    ...
    async def validate_code(self, code: str) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                logger.log("ERROR", f"Error de sintaxis en código IA: {result.stderr.decode()}", "testing")
                return False
            
            return True
        except Exception as e:
            logger.log("ERROR", f"Error validando código: {str(e)}", "testing")
            return False
    ...
    async def _run_unit_tests(self) -> Dict:
        ...
        try:
            ...
            logger.log("ERROR", f"Error en pruebas unitarias: {str(e)}", "testing")
            return {
                "passed": False,
                "error": str(e)
            }
    ...
    async def _run_integration_tests(self) -> Dict:
        ...
        try:
            ...
            logger.log("ERROR", f"Error en pruebas de integración: {str(e)}", "testing")
            return {
                "passed": False,
                "error": str(e)
            }
    ...
    async def _run_security_tests(self) -> Dict:
        ...
        try:
            ...
            logger.log("ERROR", f"Error en pruebas de seguridad: {str(e)}", "testing")
            return {
                "passed": False,
                "error": str(e)
            }
    ...
    async def _run_performance_tests(self) -> Dict:
        ...
        try:
            ...
            logger.log("ERROR", f"Error en pruebas de rendimiento: {str(e)}", "testing")
            return {
                "passed": False,
                "error": str(e)
            }
    ...
    def _get_test_coverage(self, test_type: str) -> float:
        ...
        try:
            ...
            logger.log("ERROR", f"Error obteniendo cobertura: {str(e)}", "testing")
            return 0.0
...
class DeploymentEngine:
    ...
    def __init__(self, memory: DistributedMemory, versioning: 'VersioningEngine'):
        ...
        logger.log("INFO", "DeploymentEngine iniciado", "deployment")
    ...
    async def deploy_improvements(self, improvements: List[Dict]) -> List[Dict]:
        ...
        for improvement in improvements:
            try:
                ...
                logger.log("ERROR", f"Error desplegando mejora: {str(e)}", "deployment")
        
        return deployed
    
    async def _gradual_deploy(self, improvement: Dict, percent: int) -> bool:
        """Despliegue gradual con porcentaje específico"""
        logger.log("INFO", f"Desplegando mejora al {percent}%: {improvement}", "deployment")
        ...
        try:
            ...
            if not health_ok:
                logger.log("WARNING", f"Problemas de salud detectados al {percent}% de despliegue", "deployment")
                return False
            
            return True
        except Exception as e:
            logger.log("ERROR", f"Error en despliegue gradual: {str(e)}", "deployment")
            return False
    ...
    async def _deploy_to_node(self, node_id: str, improvement: Dict) -> bool:
        ...
        try:
            ...
            logger.log("ERROR", f"Error desplegando en nodo {node_id}: {str(e)}", "deployment")
            return False
    ...
    async def _check_deployment_health(self) -> bool:
        ...
        try:
            ...
            if cpu > 90 or memory > 90:
                logger.log("WARNING", f"Uso elevado de recursos (CPU: {cpu}%, Memoria: {memory}%)", "deployment")
                return False
            
            # Verificar errores recientes
            errors = await self.memory.get_all_keys("error:*")
            if len(errors) > 10:  # Umbral de errores
                logger.log("WARNING", f"Demasiados errores detectados: {len(errors)}", "deployment")
                return False
            
            return True
        except Exception as e:
            logger.log("ERROR", f"Error verificando salud: {str(e)}", "deployment")
            return False
    
    async def _rollback_deployment(self, improvement: Dict, failed_percent: int):
        """Revertir despliegue fallido"""
        logger.log("WARNING", f"Iniciando rollback al {failed_percent}% para: {improvement}", "deployment")
        ...
        try:
            ...
            logger.log("INFO", f"Rollback completado para {improvement.get('id')}", "deployment")
        except Exception as e:
            logger.log("CRITICAL", f"Error en rollback: {str(e)}", "deployment")
    ...
    async def _revert_node(self, node_id: str, snapshot_data: Dict) -> bool:
        ...
        try:
            ...
            logger.log("ERROR", f"Error revirtiendo nodo {node_id}: {str(e)}", "deployment")
            return False
...
class ArchitectAI:
    ...
    def __init__(self, memory: DistributedMemory, evolution_engine: AutonomousEvolutionEngine):
        ...
        logger.log("INFO", "ArchitectAI iniciado", "architect")
    ...
    async def think_cycle(self):
        ...
        while True:
            try:
                ...
                logger.log("ERROR", f"Error en ciclo de planificación: {str(e)}", "architect")
    ...
    async def think(self):
        ...
        try:
            ...
            logger.log("INFO", f"{len(prioritized)} propuestas generadas, {min(3, len(prioritized))} enviadas a evolución", "architect")
            return True
        except Exception as e:
            logger.log("ERROR", f"Error en proceso de planificación: {str(e)}", "architect")
            return False
    ...
    async def execute_human_command(self, command: Dict) -> Dict:
        ...
        try:
            ...
            logger.log("CRITICAL", f"Comando humano recibido: {command['description']}", "architect")
            
            # Ejecutar comando directamente
            return await self.submit_proposal(command)
        except Exception as e:
            logger.log("ERROR", f"Error ejecutando comando humano: {str(e)}", "architect")
            return {"status": "error", "error": str(e)}
...
class ModuleManager:
    ...
    def __init__(self, security_manager: SecurityManager):
        self.security_manager = security_manager
        self.loaded_modules = {}
        logger.log("INFO", "ModuleManager iniciado", "modules")
    ...
    def load_module(self, code: str, module_name: str) -> bool:
        ...
        try:
            ...
            logger.log("INFO", f"Módulo cargado: {module_name}", "modules")
            return True
        except SyntaxError as e:
            logger.log("ERROR", f"Error de sintaxis en módulo {module_name}: {str(e)}", "modules")
            raise ModuleLoadError(f"Error de sintaxis: {str(e)}")
        except Exception as e:
            logger.log("ERROR", f"Error cargando módulo {module_name}: {str(e)}", "modules")
    ...
    def unload_module(self, module_name: str) -> bool:
        ...
        try:
            if module_name in self.loaded_modules:
                ...
                logger.log("INFO", f"Módulo descargado: {module_name}", "modules")
                return True
            return False
        except Exception as e:
            logger.log("ERROR", f"Error descargando módulo {module_name}: {str(e)}", "modules")
            return False
    ...
    def reload_module(self, module_name: str, new_code: str) -> bool:
        ...
        try:
            ...
            logger.log("ERROR", f"Error recargando módulo {module_name}: {str(e)}", "modules")
            return False
...
class RollbackManager:
    ...
    def __init__(self, fs_manager: FileSystemManager):
        ...
        logger.log("INFO", "RollbackManager iniciado", "rollback")
    ...
    async def create_snapshot(self, description: str = "") -> str:
        ...
        try:
            ...
            logger.log("INFO", f"Snapshot creado: {snapshot_id}", "rollback")
            return snapshot_id
        except Exception as e:
            logger.log("ERROR", f"Error creando snapshot: {str(e)}", "rollback")
    ...
    async def restore_snapshot(self, snapshot_id: str) -> bool:
        ...
        try:
            ...
            logger.log("WARNING", f"Sistema restaurado desde snapshot: {snapshot_id}", "rollback")
            return True
        except Exception as e:
            logger.log("CRITICAL", f"Error restaurando snapshot {snapshot_id}: {str(e)}", "rollback")
            return False
        ...
        try:
            ...
            if len(snapshots) > self.max_snapshots:
                for old_snapshot in snapshots[self.max_snapshots:]:
                    ...
                    logger.log("INFO", f"Snapshot eliminado: {old_snapshot}", "rollback")
        except Exception as e:
            logger.log("ERROR", f"Error limpiando snapshots: {str(e)}", "rollback")
...
class CommandInterpreter:
    ...
    def __init__(self, conscious_core: ConsciousCore):
        ...
        logger.log("INFO", "CommandInterpreter iniciado", "commands")
    ...
    async def execute_command(self, command: Dict) -> Dict:
        ...
        try:
            ...
            logger.log("ERROR", f"Error ejecutando comando: {str(e)}", "commands")
            return {"status": "error", "error": str(e)}
    ...
    async def _handle_restart(self) -> Dict:
        ...
        logger.log("WARNING", "Reinicio del sistema solicitado", "commands")
        return {
            "status": "success",
...
class EvolutionFinalizer:
    ...
    def __init__(self, engine: 'AutonomousEvolutionEngine', memory: DistributedMemory):
        self.engine = engine
        self.memory = memory
        logger.log("INFO", "Preparando sistema para producción", "finalizer")
    ...
    async def prepare_for_production(self):
        ...
        try:
            ...
            logger.log("SUCCESS", "Sistema ready para producción", "finalizer")
            return True
        except Exception as e:
            logger.log("ERROR", f"Error en preparación para producción: {str(e)}", "finalizer")
            return False
    ...
    async def _run_unit_tests(self) -> Dict:
        ...
        try:
            ...
            logger.log("ERROR", f"Error en pruebas unitarias: {str(e)}", "finalizer")
    ...
    async def _run_integration_tests(self) -> Dict:
        ...
        try:
            ...
            logger.log("ERROR", f"Error en pruebas de integración: {str(e)}", "finalizer")
            return {"passed": False, "error": str(e)}
    ...
    async def _run_security_tests(self) -> Dict:
        ...
        try:
            ...
            logger.log("ERROR", f"Error en pruebas de seguridad: {str(e)}", "finalizer")
            return {"passed": False, "error": str(e)}
    ...
    async def _run_performance_tests(self) -> Dict:
        ...
        try:
            ...
            logger.log("ERROR", f"Error en pruebas de rendimiento: {str(e)}", "finalizer")
            return {"passed": False, "error": str(e)}
    ...
    async def _get_test_coverage(self, test_type: str) -> float:
        ...
        try:
            ...
            logger.log("ERROR", f"Error obteniendo cobertura: {str(e)}", "finalizer")
            return 0.0
    ...
    def generate_documentation(self):
        ...
        try:
            ...
            logger.log("INFO", "Documentación generada", "finalizer")
        except Exception as e:
            logger.log("ERROR", f"Error generando documentación: {str(e)}", "finalizer")
    ...
    def setup_monitoring(self):
        ...
        try:
            ...
            logger.log("INFO", "Monitorización configurada", "finalizer")
        except Exception as e:
            logger.log("ERROR", f"Error configurando monitorización: {str(e)}", "finalizer")
    ...
    def perform_security_audit(self):
        ...
        try:
            ...
            logger.log("INFO", "Auditoría de seguridad completada", "finalizer")
        except Exception as e:
            logger.log("ERROR", f"Error en auditoría de seguridad: {str(e)}", "finalizer")

# ===================== INICIALIZACIÓN DEL SISTEMA =====================
async def system_init():
    """Inicialización completa del sistema LEGIÓN OMEGA"""
    logger.log("INFO", "Iniciando sistema LEGIÓN OMEGA", "core")
    ...
    try:
        ...
        NOMAD_MODE = True
        logger.log("WARNING", "Modo Nómada activado: sin conexión", "core")
    ...
    logger.log("INFO", "Sistema completamente inicializado", "core")
...
async def main():
    ...
    try:
        ...
        logger.log("SUCCESS", "LEGIÓN OMEGA operativo", "core")
        ...
        logger.log("INFO", "Apagando sistema LEGIÓN OMEGA", "core")
    except Exception as e:
        logger.log("CRITICAL", f"Error fatal: {str(e)}", "core")
